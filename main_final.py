...
X_500test, y_500test = test_500df["text"], test_500df["author"]
    ...
    #
    pipe_tfidf_rfc = Pipeline([('tfidf', TfidfVectorizer(ngram_range=(1, 2), stop_words=[".", "...", "!", "?"],max_df=0.8)),
                               ('rfc', RandomForestClassifier(class_weight='balanced', random_state=15))])
    #
    ...
    #                                                 random_state=15))])
    #
    pipe_tfidf_mlpc = Pipeline([('tfidf', TfidfVectorizer(ngram_range=(1, 2), stop_words=[".", "...", "!", "?"], max_df=0.8)),
                                ('mlpc', MLPClassifier(hidden_layer_sizes=(50, 50), learning_rate="adaptive",
                                                       random_state=15))])
    #
    ...
    # #
    grid_params_tfidf_rfc = [{"tfidf__binary": [True, False],
                              "rfc__n_estimators": np.arange(100, 400, 100),
                              "rfc__max_features": ['sqrt', 'log2'],
                              "rfc__criterion": ["gini","entropy"]}]
    ...
    #
    grid_params_tfidf_mlpc = [{"tfidf__binary": [True, False],
                               "mlpc__activation": ['tanh', 'relu'],
                               'mlpc__alpha': [0.0001, 0.05]}]
    #
    ...
    grids = [gs_tfidf_rfc,gs_tfidf_mlpc]
    ...
    grid_labels = ["tfidf_rfc","tfidf_mlpc"]
